...
public class Player_Jump : MonoBehaviour
{
    ...
    private Player_GroundDetection ground;
    ...
    private void calculateGravity()
    {
        ...
        if (body.velocity.y > 0.01f)
        {
            if (onGround)
            {
                //Don't change if stood on something like a platform
        ...
        else if (body.velocity.y < -0.01f)
        {
            ...
            if (onGround)
            //Don't change it if stood on something (such as a moving platform)
            {
                gravMultiplier = defaultGravityScale;
            }
            else
    ...
    private void DoAJump()
    {
        ...
        if (onGround || (coyoteTimeCounter > 0.03f && coyoteTimeCounter < coyoteTime) || canJumpAgain)
        {
            ...
            //If the player is moving up or down when they jump (such as when doing a double jump), change the jumpSpeed;
